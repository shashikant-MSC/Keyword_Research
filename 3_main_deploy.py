# ============================================
# FAST STREAMLIT APP USING PRECOMPUTED DATA
# ============================================
import os
import re
import json
...
def compute_similarity_views(keyword_vector: np.ndarray, df: pd.DataFrame,
    ...
    # Vectorized cosine similarity: sim = (EA*v) / (||E|| * ||v||)
...
# ---------- STREAMLIT UI ----------
def main():
    st.title("Multi-day Keyword Ranking (FAST Version: Precomputed Embeddings)")
    st.write(
    ...
    if st.button("Run Analysis"):
        with st.spinner("Running analysis..."):
            similarity_view, ranked_display, final_top20, word_counts = process_keyword(keyword_input)

        st.subheader("Top results per file (first 1000 rows)")
        st.dataframe(similarity_view)

        st.subheader("Top 100 after MSC ranking")
        st.dataframe(ranked_display)

        st.subheader("Final Top 20 (post-GPT)")
        st.dataframe(final_top20)

        st.subheader("Word repetition counts for Top 20")
